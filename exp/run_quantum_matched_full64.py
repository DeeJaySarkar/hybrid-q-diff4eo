"""
Run the 5-qubit quantum_matched model (StronglyEntanglingLayers, 75 params)
on the full 64x64 dataset, seed 1, 3000 epochs.
Best-val checkpoint with EMA for testing. Archived.
"""
import json, os, sys, glob, shutil, subprocess, re
...
DATA_ROOT = os.environ.get("CTGAN_DATA_64", "data/CTGAN_full_64/CTGAN/Sen2_MTC/dataset")
SEED = 1
N_EPOCH = 3000
TAG = "quantum_matched_full64_seed1"
...
cfg["train"]["val_epoch"] = N_EPOCH * 10
...
# --- train ---
print(f"### TRAINING {TAG} | {N_EPOCH} epochs | seed {SEED} | full 64x64 | 5 qubits StronglyEntangling ###")
before = exp_set()
...
if os.path.exists(ema_src):
    ...
    ckpt_prefix = os.path.join(os.path.abspath(ckpt_dir), f"{best_ep}ema")
else:
    ckpt_prefix = os.path.join(os.path.abspath(ckpt_dir), str(best_ep))
...
# --- archive ---
archive = os.path.join(HERE, "archive", "quantum_matched_full64_ep3000_seed1")
os.makedirs(os.path.join(archive, "QDiffCR", "experiments"), exist_ok=True)
for d in exp_set():
    if TAG in os.path.basename(d):
