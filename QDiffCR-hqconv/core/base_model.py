...
class BaseModel():
    ...
    def train(self):
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            ''' periodic lightweight validation LOSS (train-vs-val overfitting curve) '''
            val_loss_epoch = self.opt['train']['val_loss_epoch']
            if val_loss_epoch and self.epoch % val_loss_epoch == 0 and self.val_loader is not None:
                val_loss = self.val_loss_step()
                if val_loss is not None:
                    self.logger.info('val/mse_loss: {}\t'.format(val_loss))
                    self.logger.info('val_epoch: {}\t'.format(self.epoch))
                    if self.writer is not None:
                        self.writer.set_iter(self.epoch, self.iter, phase='val')
                        self.writer.add_scalar('mse_loss', val_loss)
