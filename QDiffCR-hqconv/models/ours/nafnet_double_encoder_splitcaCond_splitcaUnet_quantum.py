...
class CondNAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        self.sca_avg = nn.Sequential(
            ...
            nn.Conv2d(in_channels=dw_channel // 4, out_channels=dw_channel // 4, kernel_size=1, padding=0, stride=1, groups=1, bias=True),
        )
...
class UNet(nn.Module):
    def __init__(
        ...
        self.cond_intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        ...
        if q_cfg.get("enabled", True):
            ...
            if mode == "classical_control":
                ...
                self.quantum_bottleneck = QuantumMatchedLayer(chan, n_qubits=5, n_layers=5)
                ...
                # HQConv placed at encoder stage (not bottleneck)
                enc_stage = q_cfg.get("encoder_stage", 2)
                hqconv_channels = width * (2 ** enc_stage)
                self.quantum_bottleneck = None  # no bottleneck quantum
                self.hqconv_layer = HQConvQuanvLayer(
                    channels=hqconv_channels, n_qubits=8, n_layers=q_cfg.get("n_layers", 3),
                    quantum_channels=2)
                self.hqconv_stage = enc_stage
            else:
                self.quantum_bottleneck = QuantumBottleneckLayer(chan, n_qubits=n_qubits, n_layers=n_layers)
        else:
            self.quantum_bottleneck = None

        if not hasattr(self, 'hqconv_layer'):
            self.hqconv_layer = None
            self.hqconv_stage = None
    ...
    def forward(self, inp, gammas):
        ...
        for encoder, down, cond_encoder, cond_down in zip(self.encoders, self.downs, self.cond_encoders, self.cond_downs):
            ...
            x = x + tmp_cond
            # Apply HQConv quanvolution at the specified encoder stage
            if self.hqconv_layer is not None and stage_idx == self.hqconv_stage:
                x = self.hqconv_layer(x)
            encs.append(x)
            x = down(x)
            cond = cond_down(cond)
            stage_idx += 1
        ...
        # Quantum bottleneck
        if self.quantum_bottleneck is not None:
            x = self.quantum_bottleneck(x)
