...
class Palette(BaseModel):
    ...
    def val_loss_step(self):
        """Lightweight validation LOSS (same MSE-on-x0 objective as training,
        no backprop, no full sampling). Uses a fixed RNG seed each call so the
        curve reflects model improvement rather than noise-sampling variance.
        Returns the mean validation loss over the val set."""
