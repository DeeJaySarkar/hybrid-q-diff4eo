...
class Sen2_MTC_New_Multi(data.Dataset):
    def __init__(self, data_root, mode='train', frame_stride=1, frame_offset=0, image_size=None):
        ...
        for tile in self.tile_list:
            ...
            # Deterministic, reproducible frame subsampling: sort then stride.
            image_name_list = sorted(image_name_list)
            if self.frame_stride > 1 or self.frame_offset:
                image_name_list = image_name_list[self.frame_offset::self.frame_stride]
    ...
    def image_read(self, image_path):
        ...
        # Optional downscale to a target square resolution (source is 256x256).
