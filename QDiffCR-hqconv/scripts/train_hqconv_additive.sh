#!/bin/bash
# Train HQConv ADDITIVE variant (encoder stage 3, 8×8 spatial, 16 patches)
#
# This places the 8-qubit HQConv quanvolutional layer at encoder stage 3,
# processing 2×2 patches over the 8×8 feature map (16 quantum circuit calls
# per forward pass).
#
# Usage:
#   ./scripts/train_hqconv_additive.sh [--seed SEED] [--epochs N] [--batch BATCH] [--data PATH]
#
# Examples:
#   ./scripts/train_hqconv_additive.sh                         # defaults: seed=1, 3000 epochs, batch=2
#   ./scripts/train_hqconv_additive.sh --seed 42 --epochs 2000
#   ./scripts/train_hqconv_additive.sh --data /path/to/CTGAN_full_64/CTGAN/Sen2_MTC/dataset
...
TAG="hqconv_additive_${IMAGE_SIZE}x${IMAGE_SIZE}_seed${SEED}"
CFG="config/_pipe/${TAG}_train.json"

echo "=== HQConv Additive (Encoder Stage 3) ==="
echo "  Seed:       $SEED"
echo "  Epochs:     $N_EPOCH"
